...
public class MongoStore implements ChecksStore, AlertsStore, SubscriptionsStore, PermissionsStore, UserStore {
	...
	@Inject
	public MongoStore(PasswordEncoder passwordEncoder, @Value("${admin.username}") String adminUsername,
		...
		try {
			...
			if (seyrenConfig.getDatabaseSocketTimeout() > 0) {
				...
				ServerAddress mongoAddress = new ServerAddress(mongoClientUri.getURI());
				mongoClient = new MongoClient(mongoAddress, clientOptions.build());
			} else {
	...
	private void bootstrapMongo() {
		LOGGER.info(
				"Bootstrapping Mongo indexes. Depending on the number of checks and alerts you've got it may take a little while.");
		try {
			createIndices();
			removeOldIndices();
			...
			LOGGER.error(
					"Failure while bootstrapping Mongo indexes.\n"
							...
							+ "Please correct the problem by removing the clash. If it's something else, please let us know on Github!",
					e);
			throw new RuntimeException(
					"Failed to bootstrap Mongo indexes. Please refer to the logs for more information.", e);
		}
	...
	private void removeOldIndices() {
		...
		try {
			...
			if (e.getCode() != -5) {
				// -5 is the code which appears when the index doesn't exist
				// (which we're happy with, anything else is bad news)
				throw e;
			}
		}
	}
	...
	public Check saveCheck(Check check) {
		...
		DBObject partialObject = object("name", check.getName()).with("description", check.getDescription())
				.with("target", check.getTarget()).with("graphiteBaseUrl", check.getGraphiteBaseUrl())
				.with("from", Strings.emptyToNull(check.getFrom())).with("until", Strings.emptyToNull(check.getUntil()))
	...
	public SeyrenResponse<Alert> getAlerts(String checkId, int start, int items) {
		DBCursor dbc = getAlertsCollection().find(object("checkId", checkId)).sort(object("timestamp", -1)).skip(start)
				.limit(items);
